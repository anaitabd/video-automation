    ...
    def _run_step(self, *, job_id: str, step_name: str, in_state: JobState, out_state: JobState, fn: Callable[[], Any]) -> Any:
        ...
        for attempt in range(1, self.max_step_retries + 1):
            ...
            try:
                ...
                result = fn()
                metric = log_timed_event(logger, event="pipeline_step", start_time=step_start, step=step_name, attempt=attempt)
                self.state.save_step_result(job_id, step_name, {"ok": True, "result": result, "duration_ms": metric.duration_ms})
                ...
                logger.exception(
                    ...
                    extra={
                        ...
                        "details": {"step": step_name, "attempt": attempt},
                    },
                )
    ...
    def run(self, request: PipelineRequest) -> Dict[str, Any]:
        ...
        with job_context(job_id):
            ...
            try:
                ...
                logger.exception("Pipeline failed", extra={"event": "pipeline", "failure_reason": classify_failure_reason(exc)})
                raise
            finally:
                workspace.cleanup()
