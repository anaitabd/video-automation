...
class VideoAutomationOrchestrator:
    ...
    def _run_step(self, *, job_id: str, step_name: str, fn: Callable[[], Any]) -> Any:
        last_error = None
    ...
    def run(self, request: PipelineRequest) -> Dict[str, Any]:
        ...
        try:
            if "script_generation" not in step_results:
                script_data = self._run_step(job_id=job_id, step_name="script_generation", fn=lambda: self.script_generator.generate(topic=request.topic))
            ...
            step_results = self.state.get_job(job_id).get("step_results", {})
            ...
            if "audio_generation" not in step_results:
                ...
                voice_data = self._run_step(job_id=job_id, step_name="audio_generation", fn=lambda: self.voice_generator.synthesize(script=rendered_script, audio_path=str(workspace.path("audio.mp3"))))
            ...
            if "visual_descriptions" not in step_results:
                visuals = self._run_step(job_id=job_id, step_name="visual_descriptions", fn=lambda: self.visual_generator.generate(script=script))
            ...
            if "asset_fetch" not in step_results:
                assets = self._run_step(job_id=job_id, step_name="asset_fetch", fn=lambda: self.video_fetcher.fetch(topic=". ".join(visual_prompts), download_dir=str(workspace.path("scene_assets")), limit=max(3, len(visual_prompts))))
            ...
            if "render_video" not in step_results:
                ...
                video = self._run_step(job_id=job_id, step_name="render_video", fn=lambda: self.video_composer.compose(assets=fetched_assets, audio_path=audio_path, output_path=str(workspace.path("final_video.mp4")), script=request.topic))
            ...
            if "upload_video" not in step_results:
                step_results = self.state.get_job(job_id).get("step_results", {})
                final_uri = self._run_step(job_id=job_id, step_name="upload_video", fn=lambda: self.storage.upload_file(step_results["render_video"]["final_video_path"], f"{prefix}/final_video.mp4"))
            ...
            self.state.set_status(job_id, "failed", error=str(exc))
